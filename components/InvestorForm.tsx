...
import { addInvestorToList } from "@/utils/createWaitlist";
type emailSubmissionStatus = "success" | "failed" | "not-submitted";

const InvestorForm = ({ formName }: { formName: string }) => {
  ...
  const handleFormSubmission = async (event: FormEvent) => {
    ...
    addInvestorToList(fName, lName, cName, mobile, emailAddress, 3)
      .then((response) => {
        if (response.ok) {
          setSubmissionStatus("success");
          setIsSubmitting(false);
